...
class PdfController extends \BaseController
{
    ...
    {
        $html = HtmlController::guia($folio);
        $option = [
            "height" => "1107px",        // allowed units: mm, cm, in, px
            "width" => "844px",            // allowed units: mm, cm, in, px
            //"orientation" => "portrait", // portrait or landscape
        ...
        $response = Curl::to('localhost:2000/')->withData(array('html' => $html, 'options' => $option, 'rute' => '../mafer/public/pdf/T52F' . $folio . '.pdf'))->post();
        if ($response) {
            header('Content-type: application/pdf');
            header("Content-disposition: attachment;filename=T52F" . $folio . ".pdf");
            header('Content-Disposition: inline; filename=T52F' . $folio . '.pdf"');
            header('Content-Transfer-Encoding: binary');
            header('Content-Length: ' . filesize($response));
            @readfile($response);
        }
    ...
    public static function guiaBase64($folio){
        $html = HtmlController::guia($folio);
        $option = [
            "height" => "1107px",        // allowed units: mm, cm, in, px
            "width" => "844px",            // allowed units: mm, cm, in, px
